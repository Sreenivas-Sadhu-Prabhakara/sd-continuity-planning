...
| **Asset Type** | Continuity Plan |
| **Control Record** | Continuity Plan Plan |
...
| POST | `/v1/continuity-plan-plan/initiate` | Initiate |
| GET | `/v1/continuity-plan-plan` | Retrieve (list) |
| GET | `/v1/continuity-plan-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/continuity-plan-plan/{crId}/update` | Update |
| PUT | `/v1/continuity-plan-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/continuity-plan-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-continuity-planning:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-continuity-planning ./helm -n bian-risk-compliance
```

Exposed through the platform Gateway at path prefix `/sd-continuity-planning` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
